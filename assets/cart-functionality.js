...
let cart_functioality = {
  updateItemCart(line, quantity) {
  ...
  addToCart(formDataVar) {
    let formData = new FormData(formDataVar);
    let button_loading = document.querySelector(".add-to-cart-btn");
    let icon_loading = document.querySelector(".loading-overlay__spinner");
    if(!button_loading.classList.contains('notify-me')){
      button_loading.classList.add("loading");
      icon_loading.classList.remove("hidden");
      fetch(window.Shopify.routes.root + "cart/add.js", {
        ...
        .then((response) => {
          document.querySelector('.cart-notification-wrapper.notifications').classList.remove('hidden');
          document.querySelector('.cart-notification-wrapper.notifications').classList.add('animate');
          window.dispatchEvent(new Event("cart-updated"));
          button_loading.classList.remove("loading");
          icon_loading.classList.add("hidden");
          setTimeout(function(){
            document.querySelector('.cart-notification-wrapper.notifications').classList.remove('animate');
            setTimeout(function(){
              document.querySelector('.cart-notification-wrapper.notifications').classList.add('hidden');      
              document.querySelector('.cart-notification-wrapper.notifications').classList.add('animate');
            }, 600);
          }, 3000);
        })
  ...
  animate_drawer(status){
    if(status === 'showMiniCartNotification'){
      document.querySelector('.cart-notification-wrapper.notifications').classList.remove('animate');
      setTimeout(function() {
        document.querySelector('.cart-notification-wrapper.notifications').classList.add('hidden');
        document.querySelector('.cart-notification-wrapper.notifications').classList.add('animate');
      }, 600);
    }else if(status === 'showMiniCart'){
      document.querySelector('.cart-notification-wrapper.drawer').classList.remove('animate');
      setTimeout(function() {
        window.dispatchEvent(new Event("toggle-cart"));
        document.querySelector('.cart-notification-wrapper.drawer').classList.add('animate');
      }, 600);
    }
  },
  showCartNote(el){
    const elId = el.target.getAttribute('id');
    const target = document.querySelector('.cart__note');
    if(elId === 'yes'){
      target.style.maxHeight = "350px";
      document.getElementById(elId).classList.add('checked');
      document.getElementById('no').classList.remove('checked');
    }
    else{
      target.style.maxHeight = "0px";
      document.getElementById(elId).classList.add('checked');
      document.getElementById('yes').classList.remove('checked');
      document.querySelector('#Cart-note').disabled = true;
      document.querySelector('#Cart-note').value = 'Please write your message here.';
      document.querySelector('#cart_not_check').checked = false;
    }
  },
  toggle_drawers(){
    document.querySelector('.cart-notification-wrapper.notifications').classList.remove('animate');
...
window.cart_functioality = cart_functioality;
